...
| `kb-corpus/imported-300/` | 实际入库的 300 篇语料，按真实标题命名（`data/files` 下是无意义的 ID 文件名，不适合交付） |
| `kb-corpus/manifest.csv` | 语料清单：文件名、原标题、知识库内 ID、字节数、`parse_status` / `summary_status` / `enable_status` |
